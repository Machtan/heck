...
pub type ParserRules = HashMap<String, GrammarRule>;
...
fn find_parser_rules(rules: &RawRules) -> ParserRules {
    let mut rules = rules.iter().filter_map(|&(ref k, ref v)| {
        if is_token_id(k) {
...
fn main() {
    println!("Hello, world!");
    println!("Escaped quote: '{}'", ESCAPED_QUOTE);
    ...
    let rules = parse_rules(TOML_GRAMMAR).expect("Could not parse TOML grammar");
    let lexer_rules = find_lexer_rules(&rules);
    println!("Token definitions:");
    for def in &lexer_rules {
        println!("  {:?}", def);
    }
    let found = lex(TOML_FILE, &lexer_rules).expect("Lexing failed D:");
